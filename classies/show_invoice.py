...
class ShowInvoice(QWidget):
    def __init__(self, action, parent=None):
        ...
        self.period = []
        self.id_inv = []  # ID данных загружаемых из таблицы Invoice
        self.id_ser_inv = []  # ID данных загружаемых из таблицы ServiceInvoice
    ...
    def work_with_service(self, selector):
        ...
        if selector == 'add':
            ...
            self.id_selected_row = self.id_inv[index_row]
            ...
            if selector == 'dell':
                ...
                for index_row in range(0, len(services)):
                    # запоминаем id запесей в таблице service_invoice
                    self.id_ser_inv.append(services[index_row].id)
                    service_value = []
                    # наименование услуг
    ...
    def add_upt_dell(self):
        if self.win.action == 'add':
            ...
            id = self.id_inv[index_row]
            # считываем данные из таблицу Invoice
            selected_invoice = conn.query(Invoice).filter(Invoice.id == id).first()
            # считываем данные из формы
            d = self.win.date_edit.text()
            comment = self.win.comment_edit.text()
            # сохраняем в базу (таблица Acts)
            new_act = Acts(
            ...
            # считываем данные из таблицу Invoice
            selected_service_invoice = conn.query(ServiceInvoice).filter(ServiceInvoice.id == id).first()

            # определяем выделенные строчки в таблице окна с услугами
            selected_rows = self.win.table_service.selectedItems()

            prices = []
            for row in selected_rows:
                index_row = self.win.table_service.row(row)
                # ищем id текущей услуги
                id = self.id_ser_inv[index_row]
                selected_service_invoice = conn.query(ServiceInvoice).filter(ServiceInvoice.id == id).first()
                price = selected_service_invoice.price_service
                # сохраняем в базу (таблица ServiceActs)
                new_service_act = ServiceActs(
                    id_acts=last_id,
                    id_service=id,
                    amount_service=selected_service_invoice.amount_service,
                    price_service=price)
                prices.append(price)
                conn.add(new_service_act)
                conn.commit()
        ...
        #     conn.query(Invoice).filter(Invoice.id == id).update({
        #         'id_company': id_company,
        #         'date_invoice': str_to_date(d),
        #         'summ_invoice': 0,  # обновляется после сохранения услуг
        #         'comment_invoice': comment,
        ...
        #     result = conn.query(ServiceInvoice).filter(ServiceInvoice.id_invoice == id).all()
        #     id_service_list = []
        ...
        #     conn.query(ServiceInvoice).filter(ServiceInvoice.id_invoice == id).delete()
        #     conn.commit()
        #
        #     for row in range(0, rows):
        #         # считываем данные из таблицу окна с услугами
        ...
        #         conn.commit()
        #         # Обновляем сумму счёта
        #     conn.query(Invoice).filter(Invoice.id == id).update({'summ_invoice': sum(prices)})
        #     conn.commit()
        self.close()
        self.filling_table()
        self.change_period()

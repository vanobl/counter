...
class Counterpartie(QWidget):
    ...
    def work_with_counterpartie(self, selector):
        ...
        if selector == 'add':
            ...
            query_counterpartie = conn.query(Counterparties).filter_by(name_c = name_company).first()
            ...
            for ls in list:
                name_company = ls.text()
            query_company = conn.query(Counterparties).filter_by(namecompany = name_company).first()
            conn.query(Counterparties).filter(Counterparties.id==query_company.id).delete()
            conn.commit()
    ...
    def add_upt_dell(self):
        if self.win.action == 'add':
            ...
            new_company = Counterparties(self.win.edit_company.text(), self.win.edit_inn.text(), self.win.edit_ogrn.text(), self.win.edit_address.text(), self.win.edit_name_bank.text(), self.win.edit_bik.text(), self.win.edit_ks.text(), self.win.edit_rs.text())
            conn.add(new_company)
            conn.commit()
            self.win.close()
            self.filling_list()
        elif self.win.action == 'edit':
            conn.query(Counterparties).filter(Counterparties.id==self.win.label_id.text()).update({'name_c': self.win.edit_company.text(), 'inn_c': self.win.edit_inn.text(), 'ogrn_c': self.win.edit_ogrn.text(), 'adress_c': self.win.edit_address.text(), 'bank_c': self.win.edit_name_bank.text(), 'bik_c': self.win.edit_bik.text(), 'ks_c': self.win.edit_ks.text(), 'rs_c': self.win.edit_rs.text()})
            conn.commit()
            self.win.close()
            self.filling_list()
    ...
    def dell_counterpartie(self):
        ...
        for ls in list:
            name_company = ls.text()
        query_company = conn.query(Counterparties).filter_by(name_c = name_company).first()
        conn.query(Counterparties).filter(Counterparties.id==query_company.id).delete()
        conn.commit()
        self.filling_list()
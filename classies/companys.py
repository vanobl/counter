...
class Companys(QWidget):
    ...
    def work_with_company(self, selector):
        ...
        if selector == 'add':
            ...
            query_company = conn.query(Company).filter_by(namecompany = name_company).first()
    ...
    def add_upt_dell(self):
        if self.win.action == 'add':
            ...
            new_company = Company(self.win.edit_company.text(), self.win.edit_inn.text(), self.win.edit_ogrn.text(), self.win.edit_address.text())
            conn.add(new_company)
            conn.commit()
            self.win.close()
            self.filling_list()
        elif self.win.action == 'edit':
            conn.query(Company).filter(Company.id==self.win.label_id.text()).update({'namecompany': self.win.edit_company.text(), 'inn': self.win.edit_inn.text(), 'ogrn': self.win.edit_ogrn.text(), 'adress': self.win.edit_address.text()})
            conn.commit()
            self.win.close()
            self.filling_list()
